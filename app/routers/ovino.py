...
def registrar_abate(payload: AbateCreate):
    ...
    try:
        ...
        cur.execute(
            "UPDATE ovino_animais SET status=%s, updated_at=NOW() WHERE id=%s",
...
def indicadores_animal(animal_id: int):
    ...
    try:
        ...
        if movim_atual and movim_atual.get("lote_id"):
            cur.execute("SELECT COUNT(*) AS qtd FROM ovino_animais WHERE lote_id = %s AND status = 'ativo'", (movim_atual["lote_id"],))
        ...
        if confianca >= 0.5 and payload.imovel_id:
            ...
            if intent == "pesagem":
                ...
                if valor_total and qtd:
                    ...
                    dados_pendentes = {
                        "acao": "compra", "modulo": "ovino", "especie": "ovino",
                        "imovel_id": payload.imovel_id,
                    ...
                    resumo = (
                        f"Confirma a compra de {qtd} ovino(s) por R$ {float(valor_total):,.2f} "
                ...
                if valor_total and qtd:
                    ...
                    try:
                        preview = preview_venda_zootecnico(
                            cur, payload.imovel_id, "ovino", entidades.get("data_evento"), qtd
                        )
                    except ValueError:
                        preview = None
                    except Exception as e:
                        logger.error("Erro no preview de venda CV (ovino): %s", e)
                        preview = None

                    base_dados = {
                        "acao": "venda", "modulo": "ovino", "especie": "ovino",
                        "imovel_id": payload.imovel_id,
                    ...
                    if preview and preview["ambiguo"]:
                        ...
                        resumo = (
                            f"Confirma a venda de {qtd} ovino(s) por R$ {float(valor_total):,.2f}?\n\n"
                            f"Responda SIM para gravar ou NÃO para cancelar."
                        )
                    else:
                        status_log = "confirmar"
                        ...
                        resumo = (
                            f"Não encontrei histórico de compra desse ovino em Compra e Venda — "
                    ...
                    status_log = "pendente"
                    resumo = "Entendi a venda, mas não identifiquei a quantidade de animais. Quantos foram vendidos?"
...
# ── Helper ────────────────────────────────────────────────────────────────────
def _produtor_do_imovel_ovino(cur, imovel_id: int):
    cur.execute("SELECT produtor_id FROM imoveis_rurais WHERE id = %s", (imovel_id,))
    row = cur.fetchone()
    return row["produtor_id"] if row else None


def _criar_lancamento_lcdpr_ovino(conn, produtor_id, data, tipo: str, valor: float,
                                   descricao: str, origem: str = "whatsapp_ovino"):
    """Cria lançamento LCDPR em conexão própria (mesmo padrão de piscicultura.py:
    _criar_lancamento_lcdpr), para não interferir na transação principal do webhook."""
    ...
    try:
        ...
        logger.error("[OVINO] Erro ao criar lançamento LCDPR: %s", e)
        if lcdpr_conn:
            lcdpr_conn.rollback()
        return None
    finally:
