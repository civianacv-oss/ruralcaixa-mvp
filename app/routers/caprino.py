        ...
        if confianca >= 0.5 and payload.imovel_id:
            ...
            if intent == "pesagem":
                ...
                if valor_total and qtd:
                    ...
                    dados_pendentes = {
                        "acao": "compra", "modulo": "caprino", "especie": "caprino",
                        "imovel_id": payload.imovel_id,
                    ...
                    resumo = (
                        f"Confirma a compra de {qtd} caprino(s) por R$ {float(valor_total):,.2f} "
                ...
                if valor_total and qtd:
                    ...
                    try:
                        preview = preview_venda_zootecnico(
                            cur, payload.imovel_id, "caprino", entidades.get("data_evento"), qtd
                        )
                    except ValueError:
                        preview = None
                    except Exception as e:
                        logger.error("Erro no preview de venda CV (caprino): %s", e)
                        preview = None

                    base_dados = {
                        "acao": "venda", "modulo": "caprino", "especie": "caprino",
                        "imovel_id": payload.imovel_id,
                    ...
                    if preview and preview["ambiguo"]:
                        ...
                        resumo = (
                            f"Confirma a venda de {qtd} caprino(s) por R$ {float(valor_total):,.2f}?\n\n"
                            f"Responda SIM para gravar ou NÃO para cancelar."
                        )
                    else:
                        status_log = "confirmar"
                        ...
                        resumo = (
                            f"Não encontrei histórico de compra desse caprino em Compra e Venda — "
