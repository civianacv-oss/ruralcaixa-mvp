"""
RuralCaixa — app/services/ovino_tarefas.py
...
def gerar_tarefas_por_protocolo(
    ...
    cur.execute("""
        SELECT p.id FROM ovino_protocolo_manejo p
        WHERE p.fase_lote = %s
    ...
    # Busca etapas ativas
    cur.execute("""
        SELECT * FROM ovino_protocolo_etapa
...
def gerar_tarefas_cobertura(
    ...
    cur.execute("""
        SELECT id FROM ovino_protocolo_manejo
...
def concluir_tarefa(cur, tarefa_id: int, executado_por: str,
    ...
    """
    cur.execute("SELECT * FROM ovino_tarefas WHERE id = %s", (tarefa_id,))
    tarefa = cur.fetchone()
    if not tarefa:
        return None
