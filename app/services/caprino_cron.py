"""
RuralCaixa — app/services/caprino_cron.py
Cron de alertas caprinos — roda junto com o cron existente de contratos.
...
def processar_alertas_caprinos(imovel_id: int = None, dias_antecedencia: int = 1) -> dict:
    """
    ...
    try:
        ...
        for tel, dados in por_produtor.items():
            ...
            linhas = ["🐑 *RuralCaixa — Alertas Caprinos*\n"]
            ...
            linhas.append(f"\nAcesse: ruralcaixa-mvp.vercel.app/caprino")
            msg = "\n".join(linhas)
        ...
        conn.commit()
        logger.info("Cron alertas caprinos: %d enviados, %d ignorados", enviados, ignorados)
        return {"enviados": enviados, "ignorados": ignorados, "total_alertas": len(alertas)}

    except Exception as e:
        conn.rollback()
        logger.error("Erro cron alertas caprinos: %s", e, exc_info=True)
        return {"erro": str(e)}
    finally:
        conn.close()
