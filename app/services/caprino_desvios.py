"""
RuralCaixa — app/services/caprino_desvios.py
Motor de detecção de desvios zootécnicos por animal.
Chamado pelo cron a cada 30min via processar_desvios_caprinos().
"""
...
def _emitir_alerta(cur, imovel_id, animal_id, lote_id, tipo_alerta,
    ...
    cur.execute("""
        SELECT id FROM caprino_alertas
        WHERE animal_id = %s
          AND tipo_alerta = %s
          AND prioridade = %s
...
def _carregar_parametros(cur, categoria: str, imovel_id: int) -> dict:
    ...
    cur.execute("""
        ...
        FROM caprino_parametro_monitoramento
        WHERE categoria = %s
          AND ativo = TRUE
...
def _avaliar_gmd(cur, animal, params, data_ref, imovel_id, categoria) -> int:
    ...
    cur.execute("""
        SELECT peso_kg, data_pesagem FROM caprino_pesagens
...
def _avaliar_prenhez(cur, animal, params, data_ref, imovel_id) -> int:
    ...
    cur.execute("""
        SELECT data_evento FROM caprino_reproducao
        WHERE matriz_id = %s AND tipo = 'monta'
          AND data_evento <= %s
          AND NOT EXISTS (
              SELECT 1 FROM caprino_reproducao r2
              WHERE r2.matriz_id = %s
                AND r2.tipo IN ('parto','aborto')
                AND r2.data_evento > caprino_reproducao.data_evento
          )
...
def processar_desvios_caprinos(imovel_id: Optional[int] = None,
    ...
    try:
        ...
        conn.commit()
        logger.info("Desvios caprinos: %d animais, %d alertas", processados, total_alertas)
