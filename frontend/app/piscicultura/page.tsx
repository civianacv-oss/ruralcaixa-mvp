...
export default function PisciculturaPage() {
  ...
  async function salvarCompra() {
    ...
    try {
      ...
      if (r.ok) {
        setMsg("Compra registrada! Lancamento LCDPR gerado.");
        setShowCompra(false);
        setTimeout(() => loadDashboard(cicloSelecionado.id), 500);
      } else { setMsg("Erro ao salvar"); }
    } catch { setMsg("Erro de conexão"); }
    setSaving(false);
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div style={{padding:"16px 24px 0",display:"flex",gap:8}}>
        ...
        <button onClick={() => window.history.back()} style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:13,color:"#5a8a3a",fontWeight:600,padding:"6px 14px",background:"#fff",borderRadius:8,border:"1px solid #d0e8c0",boxShadow:"0 1px 3px rgba(0,0,0,0.06)",cursor:"pointer"}}>← Voltar</button>
      </div>
      {/* Header */}
              ...
              <div className="flex gap-1 border-b border-gray-200 mb-6">
                {(["dashboard", "diario", "biometria"] as const).map(t => (
                  <button key={t} onClick={() => setTabDetalhe(t)}
                    ...
                    {t === "dashboard" ? "Resumo" : t === "diario" ? "Registros Diários" : "Biometrias"}
                  </button>
                ))}
              </div>
