...
def run():
    ...
    print("=" * 70)
    cur.execute("""
        SELECT id, nome, cpf, api_token, telegram_chat_id, telefone
        FROM produtores
        WHERE id = 6
    """)
    row = cur.fetchone()
    ...
    print("=" * 70)
    cur.execute("""
        SELECT p.imovel_id, i.nome AS nome_imovel, p.percentual
    ...
    print()
    print("=" * 70)
    print("2b) Migration 023 (tipo_vinculo_participacoes) esta registrada?")
    print("=" * 70)
    try:
        cur.execute("SELECT id, description, applied_at FROM schema_migrations WHERE id LIKE %s", ('%023%',))
        rows2b = cur.fetchall()
        if rows2b:
            for r in rows2b:
                print(f"  {dict(r)}")
        else:
            print("  Nenhum registro de migration contendo '023' em schema_migrations.")
    except Exception as e:
        print(f"  Erro ao consultar schema_migrations: {e}")
        conn.rollback()
    ...
    print("=" * 70)
    cur.execute("SELECT id, nome, area_total, produtor_id FROM imoveis_rurais ORDER BY id")
    for r in cur.fetchall():
        print(f"  {dict(r)}")

    print()
    print("=" * 70)
    print("3b) Algum imovel tem produtor_id=6 diretamente (vinculo antigo, pre-participacoes)?")
    print("=" * 70)
    cur.execute("SELECT id, nome, produtor_id FROM imoveis_rurais WHERE produtor_id = 6")
    rows3b = cur.fetchall()
    if rows3b:
        for r in rows3b:
            print(f"  {dict(r)}")
    ...
    print("=" * 70)
    cur.execute("""
        SELECT fazenda_id, COUNT(*) AS qtd, MIN(data_movim) AS mais_antigo, MAX(data_movim) AS mais_recente
        FROM movimentacoes_insumo
        WHERE fazenda_id IN (SELECT imovel_id FROM participacoes_imovel WHERE produtor_id = 6)
          AND tipo = 'uso'
        GROUP BY fazenda_id
    """)
    rows4 = cur.fetchall()
