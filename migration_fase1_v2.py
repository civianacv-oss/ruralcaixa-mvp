"""
Migration Fase 1 (v2) - com retry automatico e reconexao.
Lida com quedas de conexao intermitentes reconectando e tentando de novo.

COMO USAR:
1. Salve em C:\\ruralcaixa\\ruralcaixa-mvp\\
2. python migration_fase1_v2.py
3. Cada operacao tenta ate 5 vezes, reconectando se a conexao cair
4. No final, mostra a conferencia e pergunta se pode commitar
"""
...
def executar_com_retry(sql, params=None, descricao=""):
    """Abre uma conexao NOVA para cada operacao, executa, commita, fecha.
    Isso evita manter uma transacao longa aberta (que e mais vulneravel
    a quedas), e cada passo fica salvo permanentemente assim que roda
    com sucesso."""
    for tentativa in range(1, MAX_TENTATIVAS + 1):
        ...
        try:
            ...
            cur.close()
            conn.close()
            print(f"  [OK] {descricao} (tentativa {tentativa}, linhas afetadas: {rowcount})")
            return rowcount
        except Exception as e:
            print(f"  [FALHOU tentativa {tentativa}/{MAX_TENTATIVAS}] {descricao}: {e}")
            if conn:
                try:
                    conn.close()
                except Exception:
            ...
            if tentativa < MAX_TENTATIVAS:
                ...
                raise RuntimeError(f"Falhou apos {MAX_TENTATIVAS} tentativas: {descricao}")


def consultar_com_retry(sql, params=None, descricao=""):
    for tentativa in range(1, MAX_TENTATIVAS + 1):
        conn = None
        try:
            conn = conectar()
...
CODIGO_CONTA_MAP = {
    '3.1.3.1': ['2ed522fb-411b-43fc-96a3-321014a1e815','7c4e65c5-6a76-4810-b822-ff239c23c86e',
        ...
        '2a829341-e86d-47b4-b5cf-a60d997d28ed','26b09133-6045-4ddc-8779-48a8363189c8',
        '8972a44c-d246-43b7-a344-09f5f511d198','cd5861c8-fdfb-4078-89e7-181c16a7e1e4',
        'e55ef413-102d-467b-bfa5-0fb533a2ea82'],
    '3.1.3.2': ['bc14deac-9ddd-43c0-9739-aeaba20448f8','b1b81762-2aed-44d2-9949-4ff64c8a05a9',
    ...
    '3.1.3.3': ['3450c0ee-ffdb-4fd5-aed7-01c12caf44a7','7fba3b74-5443-4168-a847-7bdc804cab7f',
    ...
    '3.1.4.1': ['a51f71aa-013e-4e6b-afea-d4ed0169582d','f9f18097-f8c6-403e-9927-2b63c1c8a67d',
        ...
        '1863b00f-009d-4dd4-a514-85f4fa86b3f1','2e844c3a-4cd5-435b-9588-c8afa9a47288'],
    '3.1.5.1': ['0b3f81ae-d4e5-4581-aa86-f2dd53eba233','76d17f64-c476-4c8e-b065-4a42da5d842f',
        '2e9ccf72-8415-4a86-b1e9-a093c3c0947f','69921897-0623-4857-9738-a09306d27076'],
    '3.1.5.2': ['d34560fb-dc4e-4527-abd5-9c5aed490a21','b898e927-4db7-4c2a-89ae-c0ec1eba5287',
    ...
    '3.1.6.1': ['0be75052-ac9b-4b37-a7b4-b1eb39a8d1e8','66e4a5de-3025-4637-9be4-c1d0eacb2d73',
        'e07b6e0b-52b2-4eac-8de3-3c54dfc8f561','15e0e502-b18c-4d47-b5b2-ab2d16865f71'],
    '3.1.7.1': ['1896e57f-768c-4d35-9789-9abc47c8cc46','04fa3f84-8131-4b18-be00-c5e613d4792f',
    ...
    '3.1.8.1': ['0173b4ae-825a-47e4-9062-714ce3326356','d1cf236e-b0bd-4ef2-b967-8498302545c2',
    ...
    '3.1.9.1': ['d8079501-0b3d-4659-bd19-c4cfa677d6ca'],
    '2.1.1': ['456d01f4-e0b9-48e4-8b71-5ab8021fb50f','4a392d28-1823-44ed-8866-c8b95e2ab29f',
    ...
    '4.1.1': ['fc881d7c-c925-4c1b-b2e9-ff548de0a24c','f9e4f62d-221a-46bb-85f2-4694d03e31ff',
    ...
    '3.1.99': ['eddc7ef5-bc9b-4a87-ad36-d2c3d0b21eb8','c1283fb6-0e2b-4698-92f6-e8a37358abe5',
...
def main():
    print("="*60)
    print("MIGRATION FASE 1 (v2 - com retry automatico)")
    print("="*60)

    print("\n[1/3] Corrigindo tipo (14 itens)...")
    executar_com_retry(
        "UPDATE subcontas SET tipo = 'DESPESA' WHERE id = ANY(%s::uuid[])",
        (TIPO_CORRECAO_IDS,),
        "correcao de tipo"
    )

    print("\n[2/3] Adicionando coluna codigo_conta...")
    executar_com_retry(
    ...
    for codigo, ids in CODIGO_CONTA_MAP.items():
        executar_com_retry(
            "UPDATE subcontas SET codigo_conta = %s WHERE id = ANY(%s::uuid[])",
            (codigo, ids),
            f"grupo {codigo}"
        )
    ...
    tipo_ok = consultar_com_retry(
        "SELECT COUNT(*) FROM subcontas WHERE tipo = 'DESPESA' AND id = ANY(%s::uuid[])",
        (TIPO_CORRECAO_IDS,),
        "conferencia tipo"
    )[0][0]
    print(f"\nTipo corrigido (esperado 14): {tipo_ok}")

    sem_codigo = consultar_com_retry(
        "SELECT id, nome, tipo, atividade_tipo FROM subcontas WHERE codigo_conta IS NULL",
        None,
        "subcontas sem codigo"
    )
    print(f"\nSubcontas SEM codigo_conta (esperado: 2, CONTRATO/CONDOMINIO):")
    for row in sem_codigo:
        print(f"   {row}")

    dist = consultar_com_retry(
        "SELECT tipo, COUNT(*) FROM subcontas GROUP BY tipo ORDER BY tipo",
        None,
        "distribuicao tipo"
    )
    print("\nDistribuicao geral de tipo:")
    for tipo, count in dist:
        print(f"   {tipo}: {count}")
    ...
    if tipo_ok == 14 and len(sem_codigo) == 2:
        ...
        print("⚠️  ATENCAO: numeros nao batem com o esperado. Revisar antes de prosseguir.")
    print("="*60)
    print("\nComo cada UPDATE ja foi commitado individualmente (nao ha")
    print("transacao pendente), NAO ha COMMIT/ROLLBACK final a fazer.")
    print("Se os numeros acima baterem, a migration esta concluida.")
