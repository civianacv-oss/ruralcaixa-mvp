...
CREATE TABLE ovino_animais (
    ...
    mae_id          INTEGER REFERENCES ovino_animais(id),
    pai_id          INTEGER REFERENCES ovino_animais(id),
...
ALTER TABLE ovino_animais
    ADD CONSTRAINT fk_lote FOREIGN KEY (lote_id) REFERENCES ovino_lotes(id);

-- ── PESAGENS ────────────────────────────────────────────────
CREATE TABLE ovino_pesagens (
    id          SERIAL PRIMARY KEY,
    animal_id   INTEGER NOT NULL REFERENCES ovino_animais(id) ON DELETE CASCADE,
...
CREATE TABLE ovino_reproducao (
    id              SERIAL PRIMARY KEY,
    imovel_id       INTEGER NOT NULL REFERENCES imoveis(id),
    tipo            VARCHAR(30) NOT NULL
                        CHECK (tipo IN ('monta','inseminacao','parto','aborto','desmame')),
    data_evento     DATE NOT NULL,
    matriz_id       INTEGER REFERENCES ovino_animais(id),
    reprodutor_id   INTEGER REFERENCES ovino_animais(id),
    cordeiros_vivos INTEGER DEFAULT 0,
    cordeiros_mortos INTEGER DEFAULT 0,
    observacoes     TEXT,
...
CREATE TABLE ovino_saude (
    id              SERIAL PRIMARY KEY,
    imovel_id       INTEGER NOT NULL REFERENCES imoveis(id),
    animal_id       INTEGER REFERENCES ovino_animais(id),   -- NULL = lote inteiro
    lote_id         INTEGER REFERENCES ovino_lotes(id),
    tipo            VARCHAR(30) NOT NULL
...
-- ── ABATES / SAÍDAS ─────────────────────────────────────────
CREATE TABLE ovino_abates (
    id              SERIAL PRIMARY KEY,
    animal_id       INTEGER NOT NULL REFERENCES ovino_animais(id),
...
CREATE TABLE ovino_whatsapp_log (
    id              SERIAL PRIMARY KEY,
    ...
    evento_tabela   VARCHAR(40),                   -- ex: 'ovino_pesagens'
    erro_msg        TEXT,
    created_at      TIMESTAMP NOT NULL DEFAULT NOW()
);

-- ── ÍNDICES ─────────────────────────────────────────────────
CREATE INDEX idx_ovino_animais_imovel   ON ovino_animais(imovel_id);
CREATE INDEX idx_ovino_animais_status   ON ovino_animais(status);
CREATE INDEX idx_ovino_pesagens_animal  ON ovino_pesagens(animal_id, data_pesagem);
CREATE INDEX idx_ovino_saude_imovel     ON ovino_saude(imovel_id, data_evento);
CREATE INDEX idx_ovino_reprod_imovel    ON ovino_reproducao(imovel_id, data_evento);
CREATE INDEX idx_ovino_wpp_status       ON ovino_whatsapp_log(status, created_at);
...
CREATE TRIGGER trg_ovino_animais_updated
    BEFORE UPDATE ON ovino_animais
